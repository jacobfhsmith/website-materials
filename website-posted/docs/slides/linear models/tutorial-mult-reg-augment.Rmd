...
As a review, today's data come from a series of atmospheric measurements taken 
...
1. Is there sufficient evidence that barometric pressure has a linear association
...
temperature, and precipitation (simply plug into the model). Try to answer the
following question, which will require some `dplyr` manipulation:

2. Suppose the barometric pressure, temperature, and precipitation are equal to the
mean values in our dataset. What would the predicted dew point be? 

3. Is this prediction close to the mean dew point in our dataset?
...
With our model we predict a dew point of 2.38 C and note that the standard error 
for this prediction is 0.195 C.

4. How might we use these results to create interval estimates?
...
How might we evaluate the four linear model assumptions?
...
5. Do the residuals appear to be normally distributed?
...
6. Do linearity and homogeneity of error variance appear satisfied in our plot?
Why or why not?
...
7. What **is** the estimated expected change in dew point given a one mm 
increase in precipitation? (Hint: as stated before, it depends on the
temperature!)
8. Recently, atmospheric pollution in terms of fine particulate matter (PM2.5)
has been of concern. Fit a model that predicts PM2.5 levels based on current
weather patterns. 
9. Assess any assumptions needed for your linear model. 
10. Is there evidence that any weather-related variables are linearly associated 
